...
def _prepare_latents_and_count(latents, dtype, bucket_mode):
    ...
    if bucket_mode:
        ...
        logger.debug(f"Bucket mode: {num_buckets} buckets, {num_images} total samples")
        for i, lat in enumerate(latents):
            logger.debug(f"  Bucket {i}: shape {lat.shape}")
    ...
    if isinstance(latents, list):
        ...
        logger.debug(f"Latent shapes: {all_shapes}")
        if len(all_shapes) > 1:
            multi_res = True
        else:
            multi_res = False
...
def _validate_and_expand_conditioning(positive, num_images, bucket_mode):
    ...
    logger.debug(f"Total Images: {num_images}, Total Captions: {len(positive)}")
...
class TrainLoraNode(io.ComfyNode):
    ...
    def execute(
            ...
            training_dtype,
            lora_dtype,
            quantized_backward,algorithm,
            gradient_checkpointing,
            checkpoint_depth,
        offloading,existing_lora,
            bucket_mode,
        bypass_mode,
    ):
        ...
        with torch.inference_mode(False):
            ...
            comfy.model_management.load_models_gpu(
                [mp], memory_required=int(1e20), force_full_load=not offloading
            )
            torch.cuda.empty_cache()
