...
def _crop_image_with_mask(
    ...
    border = torch.cat([mask_2d[0, :], mask_2d[-1, :], mask_2d[:, 0], mask_2d[:, -1]])
    center = mask_2d[height // 4:height - height // 4, width // 4:width - width // 4]
    if float(border.mean()) > 0.5 and float(center.mean()) < 0.5:
        mask = 1.0 - mask

    if mask_offset > 0:
        radius = mask_offset
        mask = torch.nn.functional.max_pool2d(mask, kernel_size=2 * radius + 1, stride=1, padding=radius)
    elif mask_offset < 0:
        radius = -mask_offset
    ...
    foreground_pixels = (alpha_u8 > 204).nonzero()
    if foreground_pixels.numel() == 0:
        inverse_foreground = ((255 - alpha_u8) > 204).nonzero()
        if inverse_foreground.numel() > 0:
            logging.info("Trellis2 preprocess: mask bbox empty, using inverted mask.")
            mask = 1.0 - mask
            foreground_pixels = inverse_foreground
    if foreground_pixels.numel() > 0:
        y_min, x_min = foreground_pixels.min(dim=0).values.tolist()
        y_max, x_max = foreground_pixels.max(dim=0).values.tolist()
        center_y, center_x = (y_min + y_max) / 2.0, (x_min + x_max) / 2.0
        bbox_width = x_max - x_min + 1
        bbox_height = y_max - y_min + 1
        if bbox_width / max(bbox_height, 1) >= aspect_ratio:
            crop_width = int(bbox_width * pad_factor)
            crop_height = int(bbox_width / aspect_ratio * pad_factor)
        else:
            crop_height = int(bbox_height * pad_factor)
            crop_width = int(bbox_height * aspect_ratio * pad_factor)
        half_width, half_height = math.ceil(crop_width / 2), math.ceil(crop_height / 2)
        crop_x1 = int(center_x - half_width)
        crop_y1 = int(center_y - half_height)
        crop_x2 = crop_x1 + 2 * half_width
        crop_y2 = crop_y1 + 2 * half_height
    else:
    ...
    # Zero-pad out-of-bounds slices (PIL.crop semantics).
    pad_left = max(0, -crop_x1)
    pad_top = max(0, -crop_y1)
    pad_right = max(0, crop_x2 - width)
    pad_bottom = max(0, crop_y2 - height)
    if pad_left or pad_top or pad_right or pad_bottom:
        img = torch.nn.functional.pad(img, (pad_left, pad_right, pad_top, pad_bottom), value=0.0)
        mask = torch.nn.functional.pad(mask, (pad_left, pad_right, pad_top, pad_bottom), value=0.0)
        crop_x1 += pad_left
        crop_x2 += pad_left
        crop_y1 += pad_top
        crop_y2 += pad_top
    ...
    background = torch.tensor(bg_rgb, dtype=cropped_img.dtype, device=cropped_img.device).view(1, 3, 1, 1)
    composite = (cropped_img * cropped_mask + background * (1.0 - cropped_mask)).clamp(0, 1)
    return composite, crop_bbox, scene_size


class ImageCropToMask(IO.ComfyNode):
    """Crop an image to its mask and composite it onto a fixed-size background."""
        ...
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Float.Input(
                    ...
                    max=2.0,
                    step=0.01,
                    tooltip="Extra margin around the mask bounding box as a multiplier.",
                ),
                IO.Int.Input(
                    "grow_mask",
                    default=0,
                    ...
                    max=32,
                    step=1,
                    tooltip="Grow or shrink the mask by this many pixels before cropping.",
                ),
...
def _add_avif_exif_item(meta: bytes, exif_offset: int, exif_length: int, offset_delta: int) -> bytes:
    ...
    child_by_type = {box_type: (position, size, header_size) for position, size, box_type, header_size in children}
...
def _adjust_avif_chunk_offsets(moov: bytes, offset_delta: int) -> bytes:
    ...
    def adjust(start: int, end: int) -> None:
        for position, size, box_type, header_size in _bmff_boxes(output, start, end):
...
def inject_avif_metadata(path: str, metadata: dict) -> None:
    ...
    with open(path, "rb") as source:
        ...
        media_data_box = next((box for box in top_level_boxes if box[2] == b"mdat"), None)
        if (
            meta_box is None
            or media_data_box is None
            or media_data_box[0] + media_data_box[1] != file_size
            or meta_box[0] > media_data_box[0]
        ):
    ...
    exif_offset = media_data_box[0] + media_data_box[1] + offset_delta
    ...
    try:
        with os.fdopen(file_descriptor, "wb") as destination, open(path, "rb") as source:
            for position, size, box_type, header_size, size_field in top_level_boxes:
                source.seek(position)
                if box_type == b"ftyp":
                    ftyp = bytearray(source.read(size))
                    # The frontend reader recognizes the avif major brand; avis remains a sequence brand.
...
def _avif_frame(image: torch.Tensor, bit_depth: str, colorspace: str, pixel_format: str) -> av.VideoFrame:
    ...
    if num_channels not in (1, 3):
        raise ValueError(
            "AVIF saving supports 1-channel grayscale and 3-channel RGB images; "
            "PyAV's SVT-AV1 encoder does not support alpha."
        )
