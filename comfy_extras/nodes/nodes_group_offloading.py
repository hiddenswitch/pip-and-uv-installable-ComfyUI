...
_DIFFUSERS_IMPORT_ERROR: Optional[Exception] = None

try:
    from diffusers import HookRegistry
    from diffusers.hooks import apply_group_offloading, apply_layerwise_casting, ModelHook
except Exception as exc:  # pragma: no cover - depends on torch/diffusers runtime matrix
    _DIFFUSERS_IMPORT_ERROR = exc
    logger.warning("group offloading nodes disabled", exc_info=exc)


if _DIFFUSERS_IMPORT_ERROR is None:
