...
from comfy_api_nodes.util._helpers import _retry_after_wait
from comfy_api_nodes.util._helpers import default_base_url
from comfy_api_nodes.util._helpers import diagnose_connectivity
...
def _parse_estimate_seconds(header_value: str | None) -> int | None:
    ...
    try:
        ...
        logger.debug("Ignoring malformed estimate header value: %r", header_value)
        return None
    if not 1 <= seconds <= 86400:
        logger.debug("Ignoring out-of-range estimate header value: %r", header_value)
        return None
    return seconds
...
def _maybe_remember_server_estimate(node_cls: type[IO.ComfyNode], headers: Mapping[str, str]) -> None:
    ...
    logger.debug("Server duration estimate: p50=%ss, p90=%s, source=%s", p50, p90, source)
        ...
        while consumed_attempts < max_poll_attempts:
            ...
            if progress_extractor:
                try:
                    ...
                    logger.error("Progress extraction failed: %s", e)
                    new_progress = None
        ...
        try:
            ...
            async with resp:
                if keyed and resp.headers.get(_IDEMPOTENT_REPLAYED_HEADER):
                    logger.info("Server replayed the response of a previous attempt for %s %s", method, url)
                if resp.status >= 400:
                    try:
