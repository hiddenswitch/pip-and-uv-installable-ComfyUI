...
## Custom Node Pip Facade

This fork can serve a local PEP 503 "simple" index for selected ComfyUI ecosystem custom nodes. The facade builds wheels on demand from ComfyUI-Manager registry metadata, injects dependency metadata, and still loads the actual nodes through the existing vanilla custom-node mitigation layer.

Start the facade server from your ComfyUI checkout:
...
Then install custom nodes from another shell:
