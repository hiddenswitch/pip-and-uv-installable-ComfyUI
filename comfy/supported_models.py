...
class ZImage(Lumina2):
    ...
    def __init__(self, unet_config):
        super().__init__(unet_config)
        if extended_fp16_support() and unet_config.get("allow_fp16", False):
...
class WAN22_WanDancer(WAN21_T2V):
    ...
    def get_model(self, state_dict, prefix="", device=None):
        return model_base.WAN22_WanDancer(self, image_to_video=True, device=device)
...
class HiDreamO1(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(
            hidream_o1.HiDreamO1Tokenizer,
            hidream_o1.HiDreamO1TE,
        )
...
class SenseNovaU15(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(
            sensenova.SenseNovaTokenizer,
            sensenova.SenseNovaTextEncoder,
        )
...
class SeedVR2(supported_models_base.BASE):
    ...
    latent_format = latent_formats.SeedVR2
    ...
    def set_inference_dtype(self, dtype, manual_cast_dtype, device=None):
        if (
            ...
            and model_management.should_use_bf16(device)
        ):
...
class ACEStep15(supported_models_base.BASE):
    ...
    latent_format = latent_formats.ACEAudio15
...
class LongCatImage(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        hunyuan_detect = hunyuan_video.llama_detect(state_dict, "{}qwen25_7b.transformer.".format(pref))
        return supported_models_base.ClipTarget(longcat_image.LongCatImageTokenizer, longcat_image.te(**hunyuan_detect))
...
class ErnieImage(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        hunyuan_detect = hunyuan_video.llama_detect(state_dict, "{}ministral3_3b.transformer.".format(pref))
        return supported_models_base.ClipTarget(ernie.ErnieTokenizer, ernie.te(**hunyuan_detect))
...
class CogVideoX_T2V(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(cogvideo.CogVideoXT5Tokenizer, sd3_clip.T5XXLModel)
