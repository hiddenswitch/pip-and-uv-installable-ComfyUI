...
from .database.queries import create_content_reporting_insert
from .database.queries import create_record
from .database.queries import mark_content_missing
from .event_log import emit
from .event_log import error_type
from .helpers import sql_path_under_prefix
from .helpers import to_stored_hash
from .lifecycle import get_excluded_scan_roots
from .scanner_admission import PARTIAL_DOWNLOAD_EXTENSIONS as PARTIAL_DOWNLOAD_EXTENSIONS
from .scanner_admission import _WATCH_LIST as _WATCH_LIST
from .scanner_admission import _WatchEntry as _WatchEntry
from .scanner_admission import _should_skip_extension
from .scanner_admission import _two_stat_admit
from .scanner_admission import tick_watch_list as tick_watch_list
from .scanner_changes import clear_pending_verifications
from .scanner_changes import detect_content_change
from .scanner_changes import drain_pending_verifications
from .scanner_changes import is_path_under_prefixes
from .scanner_changes import live_contents_under_prefixes
from .scanner_changes import pending_recovery_count
from .scanner_changes import recover_missing_content
from .services.file_utils import get_mtime_ns
from .services.file_utils import is_visible
from .services.file_utils import list_files_recursively
from .services.image_dimensions import extract_image_dimensions
from .services.ingest import _discard_unreferenced_content
...
def _log_scan_error(phase: str, error: OSError) -> None:
    ...
    logger.warning("Asset scan error: phase=%s error_type=%s", phase, error_type)
...
def sync_prefixes_with_filesystem(
    ...
    for content in live_contents_under_prefixes(session, prefixes):
        try:
            ...
            logger.debug("Permission denied accessing %s", content.path)
        except OSError as e:
            _log_scan_error("reference_stat", e)
            logger.debug("OSError checking %s: %s", content.path, e)
            mark_content_missing(session, content.id)
        else:
...
def sync_root_safely(
    ...
    try:
        ...
        logger.exception("fast DB scan failed for %s: %s", root, exc)
        emit(
...
def mark_missing_outside_prefixes_safely(prefixes: list[str]) -> int:
    ...
    try:
        ...
        logger.exception("marking missing assets failed: %s", exc)
        emit(
            ...
            try:
                with session.begin_nested():
                    try:
                        ...
                        logger.warning("Skipping vanished asset during scan: %s", path)
                        continue
                    try:
                ...
                logger.warning("Skipping asset whose row conflicts during scan: %s", path)
                continue
    except Exception:
        session.rollback()
    ...
    with create_session() as sess:
        for row in rows:
            ...
            try:
                ...
                logger.warning("Failed to enrich %s: %s", row.file_path, exc)
                sess.rollback()
                failed_ids.append(row.record_id)
