...
from ..database.models import AssetTag
from ..database.queries.records import create_content_reporting_insert
from ..database.queries.records import create_record
from ..database.queries.records import mark_content_missing
...
from .lookup import refresh_qualified_content
from .metadata_extract import extract_file_metadata
from .path_utils import compute_loader_path
from .path_utils import get_name_and_tags_from_asset_path
from .path_utils import resolve_destination_from_tags
from .path_utils import validate_path_within_base
from .schemas import AssetData
from .schemas import ReferenceData
from .schemas import RegisteredAsset
...
def _discard_unreferenced_content(session: Session, content_id: str) -> None:
    ...
    try:
        ...
        logger.exception("Failed to discard orphan content %s", content_id)
...
def create_from_hash(
    ...
    with create_session() as session:
        ...
        if content is None:
            logger.warning("create_from_hash: no asset found for hash %s", hash_str)
            return None
        content_id = content.id
        if not claim_qualified_content(session, content_id, stored_hash):
            session.rollback()
...
def register_cached_output(
    ...
    try:
        ...
        logger.exception("Failed to register cached output: %s", locator)
        return None
...
def register_executed_output(
    ...
    try:
        ...
        logger.exception("Failed to register executed output: %s", locator)
        return None
