...
def pin_memory(module, subset="weights", size=None):
    ...
    if priority is None:
        priority = utils.bit_reverse_range(counter[0], 16)
        counter[0] += 1
        module_pin["balancer_priority"] = priority

    memory_management.extra_ram_release(memory_management.RAM_CACHE_HEADROOM)
    if (not model_management.ensure_pin_budget(size, loaded=loaded) or
        not model_management.ensure_pin_registerable(registerable_size, evict_active=not fast)):
    ...
    try:
        ...
        if torch.cuda.cudart().cudaHostRegister(pin.data_ptr(), size, 1) != 0:
            model_management.discard_cuda_async_error()
            model_management.free_registrations(size, evict_active=not fast)
            if torch.cuda.cudart().cudaHostRegister(pin.data_ptr(), size, 1) != 0:
                model_management.discard_cuda_async_error()
                del pin
