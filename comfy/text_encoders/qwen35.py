...
from .llama import Llama2_
from .llama import MLP
from .llama import RMSNorm
from .llama import apply_penalty
from .llama import apply_rope
from .llama import penalty_active
from .llama import precompute_freqs_cis
from .llama import rope_matrix
...
class GatedDeltaNet(nn.Module):
    ...
    def forward(self, x, past_key_value=None, **kwargs):
        ...
        if use_recurrent:
            ...
            g_decay = -cast_to_device(self.A_log, x.device, torch.float32).exp()
            dt_bias = cast_to_device(self.dt_bias, x.device, torch.float32)
            if past_key_value is not None:
                past_key_value.g_decay = g_decay
                past_key_value.dt_bias = dt_bias
                past_key_value.norm_weight = model_management.cast_to(self.norm.weight, dtype=x.dtype, device=x.device)
...
class Qwen35(BaseLlama, BaseGenerate, torch.nn.Module):
    ...
    def _generate_mtp(self, embeds, max_length, stop_tokens, sampling=None, fixed_depth=None):
        ...
        dt = torch.bfloat16 if model_management.should_use_bf16(device) else torch.float32
        embeds = embeds.to(dt)
        if embeds.ndim == 2:
        ...
        pos = embeds.shape[1]
        progress = utils.ProgressBar(max_length)
        ...
        use_graph = (device.type == "cuda"
                     and model_management.NUM_STREAMS > 0
                     and not model_management.args.disable_cuda_graphs)
        compile_allocations = use_graph and self.model.graph_dynamic_vbar_blocks and model_prefetch.malloc_graph_enabled(device)
        draft_state = {}
        ...
        try:
            if pinned:
                model_prefetch.pin_modules(pinned, device, dt)
            if use_graph and len(ids) < max_length and ids[-1] not in stop_tokens:
                draft_capture()
            while len(ids) < max_length and ids[-1] not in stop_tokens:
                with (model_prefetch.malloc_graph_scope(device) if compile_allocations else contextlib.nullcontext()):
                    accepts, commit = step()
                ...
                if probe is not None:
                    ...
                    if probe[0] == 32:
                        ...
                        if (sampling is None and max_length - len(ids) > 512 and 1 + probe[1] / probe[0] >= 2.2
                                and 5 * snapshot_bytes < model_management.get_free_memory(device)):
            ...
            drop_draft_graph()
            if pinned:
                model_prefetch.cleanup_prefetched_modules(None, pinned)
        return ids
...
class Qwen35ImageTokenizer(sd1_clip.SD1Tokenizer):
    ...
    def tokenize_with_weights(self, text, return_word_ids=False, llama_template=None, images=[], prevent_empty_text=False, thinking=False, system_prompt="", **kwargs):
...
class Qwen35ClipModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="hidden", layer_idx=-2, dtype=None, attention_mask=True, model_options=None, model_type="qwen35_2b", textmodel_json_config=None, mtp=False):
        ...
        if mtp:
            textmodel_json_config = {**textmodel_json_config, "mtp": True}
