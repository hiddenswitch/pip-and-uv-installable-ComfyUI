...
class Gemma3_Tokenizer():
    ...
    def tokenize_with_weights(self, text, return_word_ids=False, image=None, llama_template=None, skip_template=True, **kwargs):
        ...
        if image is None:
            ...
            s = common_upscale(samples, width, height, "area", "disabled").movedim(1, -1)
            images = [s[:, :, :, :3]]
...
class Gemma3_12BModel(sd1_clip.SDClipModel):
    ...
    def generate(self, tokens, do_sample, max_length, temperature, top_k, top_p, min_p, repetition_penalty, seed):
        ...
        normalize_image_embeddings(embeds, embeds_info, self.transformer.model.config.hidden_size ** 0.5)
...
class LTXAVTEModel(torch.nn.Module):
    ...
    def load_sd(self, sd):
        if "model.layers.47.self_attn.q_norm.weight" in sd:
            ...
            sdo = state_dict_prefix_replace(sd, {"text_embedding_projection.aggregate_embed.weight": "text_embedding_projection.weight", "text_embedding_projection.": "text_embedding_projection."}, filter_keys=True)
            if len(sdo) == 0:
                sdo = sd
            ...
            if "model.diffusion_model.audio_embeddings_connector.transformer_1d_blocks.2.attn1.to_q.bias" not in sd:  # TODO: remove
                ...
                if ww is not None:
                    if ww.shape[0] == 3840:
                        ...
                        sdv = state_dict_prefix_replace(sd, {"model.diffusion_model.video_embeddings_connector.": ""}, filter_keys=True)
                        self.video_embeddings_connector.load_state_dict(sdv, strict=False, assign=getattr(self, "can_assign_sd", False))
                        sda = state_dict_prefix_replace(sd, {"model.diffusion_model.audio_embeddings_connector.": ""}, filter_keys=True)
