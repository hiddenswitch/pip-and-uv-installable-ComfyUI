...
from ..clip_model import clip_preprocess
...
from ..ops import cast_bias_weight
from ..ops import uncast_bias_weight
...
class Llama2_(nn.Module):
    ...
    def forward(self, x, attention_mask=None, embeds=None, num_tokens=None, intermediate_output=None, final_layer_norm_intermediate=True,
        ...
        prefetch_queue = model_prefetch.make_prefetch_queue(list(self.layers), x.device, {"prefetch_dynamic_vbars": self.prefetch_dynamic_vbars and past_key_values is not None})
...
class BaseGenerate:
    ...
    def generate(self, embeds=None, do_sample=True, max_length=256, temperature=1.0, top_k=50, top_p=0.9, min_p=0.0, repetition_penalty=1.0, seed=42, stop_tokens=None, initial_tokens=[], execution_dtype=None, min_tokens=0, presence_penalty=0.0, initial_input_ids=None, position_ids=None, deepstack_embeds=None, visual_pos_masks=None, embeds_info=None):
        ...
        compile_allocations = self.model.graph_dynamic_vbar_blocks and model_prefetch.malloc_graph_enabled(device)
        decode_buffers = None
        if compile_allocations and not model_management.args.disable_cuda_graphs:
...
class Gemma3_4B_Vision(BaseLlama, BaseGenerate, torch.nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        self.vision_model = CLIPVision(config.vision_config, dtype, device, operations)
