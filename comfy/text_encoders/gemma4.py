...
class Gemma4Attention(nn.Module):
    ...
    def forward(
        ...
        if isinstance(shared_kv, FixedKV):
            ...
            xq = quant_ops.ck.apply_rope_split_half1(xq, freqs_cis)
        ...
        if shared_kv is not None:
            ...
            xv = xv.transpose(1, 2)
            xq = quant_ops.ck.apply_rope_split_half1(xq, freqs_cis)
            xk = quant_ops.ck.apply_rope_split_half1(xk, freqs_cis)
...
class TransformerBlockGemma4(nn.Module):
    ...
    def forward(self, x, attention_mask=None, freqs_cis=None, past_key_value=None, per_layer_input=None, shared_kv=None):
        ...
        x = torch.mul(x, comfy_ops.cast_to_input(self.layer_scalar, x), out=output)
...
class Gemma4Transformer(nn.Module):
    ...
    def forward(self, x, attention_mask=None, embeds=None, num_tokens=None, intermediate_output=None,
        ...
        prefetch_queue = model_prefetch.make_prefetch_queue(
        ...
        enable_graph = (decode and mask is None and self.graph_dynamic_vbar_blocks
                        ...
                        and model_management.is_device_cuda(x.device))
        decode_bias = None
        decode_masks = None
        if fixed_kv:
            prepared = set()
        ...
        if enable_graph:
            ...
            if state is None or state["key"] != state_key:
                ...
                model_management._register_cross_step(self)
            state["x"].copy_(x)
        ...
        for i, layer in enumerate(self.layers):
            ...
            model_prefetch.prefetch_queue_pop(prefetch_queue, x.device, layer, x.dtype, core=core, enable_graph=enable_graph)
        ...
        if prefetch_queue is not None:
            model_prefetch.prefetch_queue_pop(prefetch_queue, x.device, None)
...
class Gemma4UnifiedVisionEmbedder(nn.Module):
    ...
    def forward(self, pixel_values, image_position_ids):
        ...
        pos = model_management.cast_to_device(self.pos_embedding, x.device, x.dtype)
