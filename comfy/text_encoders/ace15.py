...
def _audio_logits(model, x, audio_start, audio_end, eos_token=None):
    ...
    if module.comfy_cast_weights:
        weight, _, offload_stream = ops.cast_bias_weight(module, input, offloadable=True)
    else:
    ...
    ops.uncast_bias_weight(module, weight, None, offload_stream)
...
def sample_manual_loop_no_classes(
    ...
    fixed_kv = isinstance(past_key_values[0], llama.FixedKV)

    decode_tokens = torch.empty((embeds_batch, 1), device=device, dtype=torch.long)
    decode_buffers = None
    if model.transformer.model.graph_dynamic_vbar_blocks and model_prefetch.malloc_graph_enabled(device) and not model_management.args.disable_cuda_graphs:
    ...
    try:
        for step in utils.model_trange(max_new_tokens, desc="LM sampling"):
...
def generate_audio_codes(model, positive, negative, min_tokens=1, max_tokens=1024, seed=0, cfg_scale=2.0, temperature=0.85, top_p=0.9, top_k=0, min_p=0.000):
    ...
    with ops.use_quantized_matmul(model, model.execution_device):
...
class ACE15Tokenizer(sd1_clip.SD1Tokenizer):
    ...
    def tokenize_with_weights(self, text, return_word_ids=False, **kwargs):
        ...
        out = {
            prompt_key: self.sd_tokenizer.tokenize_with_weights(
                prompt,
...
class Qwen3_4B_ACE15(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="last", layer_idx=None, dtype=None, attention_mask=True, model_options=None, textmodel_json_config=None):
        ...
        super().__init__(device=device, layer=layer, layer_idx=layer_idx, textmodel_json_config=textmodel_json_config, dtype=dtype, special_tokens={"pad": 151643}, layer_norm_hidden_state=False, model_class=llama.Qwen3_4B_ACE15_lm, enable_attention_masks=attention_mask, return_attention_masks=attention_mask, model_options=model_options)
