...
from . import supported_models, utils, memory_management
...
def detect_unet_config(state_dict, key_prefix, metadata=None):
    ...
    if shape_key in state_dict_keys or tex_key in state_dict_keys:  # Trellis2 / Pixal3D
        ...
        for submodel, name, proj_in_channels in (
            ("img2shape", "shape", 2048),
            ("shape2txt", "texture", 2048),
            ("structure_model", "structure", 1024),
        ):
            key = '{}{}.blocks.0.cross_attn.proj_linear.weight'.format(key_prefix, submodel)
            if key in state_dict_keys:
                unet_config[f"image_attn_mode_{name}"] = "proj"
                unet_config[f"proj_in_channels_{name}"] = proj_in_channels
        return unet_config
    ...
    if '{}video_patch_proj.weight'.format(key_prefix) in state_dict_keys and '{}audio_patch_proj.weight'.format(key_prefix) in state_dict_keys:  # MiniMax H3
        ...
        dit_config["latents_dim"] = state_dict['{}final_layer.video_out.weight'.format(key_prefix)].shape[0] // 4
    ...
    if all('{}{}'.format(key_prefix, k) in state_dict_keys for k in qwen_image21_keys) and any('{}transformer_blocks.0.img_mlp.{}.weight'.format(key_prefix, k) in state_dict_keys for k in ('gate_up', 'proj')):  # Qwen Image 2.1
        ...
        dit_config["in_channels"] = state_dict['{}proj_out.weight'.format(key_prefix)].shape[0]
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if 'joint_transformer_blocks.0.attn.add_k_proj.weight' in state_dict:  # AuraFlow
        ...
        sd_map = utils.z_image_to_diffusers({"n_layers": n_layers, "dim": dim}, output_prefix=output_prefix)
