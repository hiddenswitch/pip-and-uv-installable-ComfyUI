...
def prepare_empty_noise(latent_image):
    if latent_image.is_nested:
        return NestedTensor([torch.zeros_like(t, device="cpu") for t in latent_image.unbind()])
...
def fix_empty_latent_channels(model, latent_image, downscale_ratio_spacial=None, downscale_ratio_temporal=None):
    ...
    if is_empty:
        ...
        if downscale_ratio_spacial is not None:
            if downscale_ratio_spacial != latent_format.spacial_downscale_ratio:
                ...
                latent_image = utils.common_upscale(latent_image, round(latent_image.shape[-1] * ratio), round(latent_image.shape[-2] * ratio), "nearest-exact", crop="disabled")
    ...
    if is_empty and downscale_ratio_temporal is not None:
        if downscale_ratio_temporal != latent_format.temporal_downscale_ratio:
            ...
            latent_image = utils.repeat_to_batch_size(latent_image, new_t, dim=2)
...
def sample(model, noise, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, denoise=1.0, disable_noise=False, start_step=None, last_step=None, force_full_denoise=False, noise_mask=None, sigmas=None, callback=None, disable_pbar=False, seed=None):
    ...
    samples = samples.to(device=model_management.intermediate_device(), dtype=model_management.intermediate_dtype())
    return samples
