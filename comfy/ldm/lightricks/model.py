...
def apply_rotary_emb(input_tensor, freqs_cis):
    ...
    if model_management.in_training:
        ...
        output = quant_ops.ck.apply_rope_split_half1(input_tensor, rotation_matrix)
    else:
        output = quant_ops.ck.apply_rope1(input_tensor, rotation_matrix)
...
def apply_rotary_emb_qk(q, k, freqs_cis):
    ...
    if split_pe:
        q, k = quant_ops.ck.apply_rope_split_half(q, k, rotation_matrix)
    else:
        q, k = quant_ops.ck.apply_rope(q, k, rotation_matrix)
...
class CrossAttention(nn.Module):
    def __init__(
            ...
            dropout=0.0,
            attn_precision=None,
            apply_gated_attention=False,dtype=None,
            device=None,
            operations=None,
    ):
        super().__init__()
...
def interleaved_freqs_cis(freqs, pad_size):
    """Compatibility helper retained for LTX custom nodes using the old rope API."""
...
def freqs_cis_matrix(freqs, pad_size, split_mode, num_attention_heads, out_dtype):
    ...
    B, T, half_head_dim = cos_freq.shape
    cos_freq = cos_freq.reshape(B, T, num_attention_heads, half_head_dim // num_attention_heads)
    sin_freq = sin_freq.reshape(B, T, num_attention_heads, half_head_dim // num_attention_heads)
...
class LTXBaseModel(torch.nn.Module, ABC):
    ...
    def __init__(
        ...
        caption_projection_first_linear=True,dtype=None,
            device=None,
            operations=None,
            **kwargs,
    ):
...
class LTXVModel(LTXBaseModel):
    ...
    def __init__(
            ...
            timestep_scale_multiplier = 1000.0,
            caption_proj_before_connector=False,
        cross_attention_adaln=False,dtype=None,
            device=None,
            operations=None,
            **kwargs,
    ):
