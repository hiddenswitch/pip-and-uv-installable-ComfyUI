...
class AttentionPooler(nn.Module):
    ...
    def forward(self, x):
        ...
        special = model_management.cast_to(self.special_token, device=x.device, dtype=x.dtype).expand(B, T, 1, -1)
...
class FSQ(nn.Module):
    ...
    def bound(self, z):
        levels_minus_1 = (model_management.cast_to(self._levels, device=z.device, dtype=z.dtype) - 1)
    ...
    def codes_to_indices(self, zhat):
        zhat_normalized = (zhat + 1.) / (2. / (model_management.cast_to(self._levels, device=zhat.device, dtype=zhat.dtype) - 1))
        return (zhat_normalized * model_management.cast_to(self._basis, device=zhat.device, dtype=zhat.dtype)).sum(dim=-1).round().to(torch.int32)
...
class ResidualFSQ(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'soft_clamp_input_value'):
            sc_val = model_management.cast_to(self.soft_clamp_input_value, device=x.device, dtype=x.dtype)
        ...
        for layer, scale in zip(self.layers, self.scales):
            scale = model_management.cast_to(scale, device=x.device, dtype=x.dtype)
