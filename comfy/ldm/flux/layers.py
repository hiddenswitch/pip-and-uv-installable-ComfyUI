...
def modulated_norm(x, norm, scale, shift, modulation_dims=None):
    if modulation_dims is not None or model_management.in_training:
        return apply_mod(norm(x), 1 + scale, shift, modulation_dims)
    return quant_ops.ck.adaln(x, scale, shift, norm.eps)
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, attn_mask=None, modulation_dims=None, transformer_options={}) -> Tensor:
        ...
        qkv, mlp = torch.split(
            self.linear1(modulated_norm(x, self.pre_norm, mod.scale, mod.shift, modulation_dims)),
