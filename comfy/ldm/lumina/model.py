...
class JointAttention(nn.Module):
    ...
    def forward(
        ...
        if self.qk_norm and not model_management.in_training:
            q_scale, _, q_offload_stream = ops.cast_bias_weight(self.q_norm, xq, offloadable=True)
            k_scale, _, k_offload_stream = ops.cast_bias_weight(self.k_norm, xk, offloadable=True)
            ...
            if self.n_local_heads == self.n_local_kv_heads:
                xq, xk = quant_ops.ck.rms_rope(xq, xk, freqs_cis, q_scale, k_scale, epsilon)
            else:
                xq = quant_ops.ck.rms_rope1(xq, freqs_cis, q_scale, epsilon)
                xk = quant_ops.ck.rms_rope1(xk, freqs_cis, k_scale, epsilon)
...
class NextDiTPixelSpace(NextDiT):
    ...
    def forward(self, x, timesteps, context, num_tokens, attention_mask=None, **kwargs):
        ...
        neg_x0 = WrapperExecutor.new_class_executor(
            self._forward,
            self,
            get_all_wrappers(WrappersMP.DIFFUSION_MODEL, kwargs.get("transformer_options", {}))
        ).execute(x, timesteps, context, num_tokens, attention_mask, **kwargs)
