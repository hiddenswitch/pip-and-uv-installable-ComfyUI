...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.feat_scale:
            ...
            out = out + ops.cast_to_input(self.lambda_dc, out) * out_dc + ops.cast_to_input(self.lambda_hf, out) * out_hf
    ...
    def forward(
        ...
        if self.has_global_cond and global_cond is not None:
            if self.global_cond_shared_embed:
                ...
                ssg = (ops.cast_to_input(self.to_scale_shift_gate, global_cond) + global_cond).unsqueeze(1)
            else:
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.num_memory_tokens > 0:
            memory_tokens = ops.cast_to_input(self.memory_tokens, x).expand(batch, -1, -1)
