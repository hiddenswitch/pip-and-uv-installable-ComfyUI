...
class WanModel(torch.nn.Module):
    ...
    def _forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options=None, **kwargs):
        ...
        context_latents = kwargs.get("context_latents", None)
        if context_latents is not None:
            context_latents = [pad_to_patch_size(lat, self.patch_size) for lat in context_latents]
...
class SCAILWanModel(WanModel):
    ...
    def _forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options={}, pose_latents=None, ref_mask_latents=None, sam_latents=None, **kwargs):
        ...
        if ref_mask_latents is not None:  # SCAIL-2
            ref_mask_latents = pad_to_patch_size(ref_mask_latents, self.patch_size)
        if sam_latents is not None:  # SCAIL-2
            sam_latents = pad_to_patch_size(sam_latents, self.patch_size)
