...
def load_lora_for_models(model, clip, lora, strength_model, strength_clip, lora_name=None, lora_metadata=None):
    key_map = {}
...
class CLIP:
    def __init__(self, target: CLIPTarget = None, embedding_directory=None, no_init=False, textmodel_json_config=None, tokenizer_data: dict | None = None, parameters=0, state_dict=[], model_options={}, disable_dynamic=False):
...
class VAE:
    def __init__(self, sd=None, device=None, config=None, dtype=None, metadata=None, no_init=False, ckpt_name: Optional[str] = ""):
        ...
        if config is None:
            ...
            elif "student.dconv_encoder.proj_out.weight" in sd:
                sd = utils.state_dict_prefix_replace(sd, {"student.dconv_encoder.": "dconv_encoder.", "pipeline.": "decoder_model."})
                sd = {key: value for key, value in sd.items() if not key.startswith("decoder_model.y_embedder.encoder.") and not key.startswith("decoder_model.y_embedder.bottleneck.")}
                self.first_stage_model = MageVAE()
            ...
            # Hunyuan 3d v2 2.0 & 2.1
                ...
                self.first_stage_model = SA3AudioVAE(**config)
...
def load_clip_model_patcher(ckpt_paths, embedding_directory=None, clip_type=CLIPType.STABLE_DIFFUSION, textmodel_json_config: str | dict | None = None, model_options=None, disable_dynamic=False):
    if model_options is None:
        model_options = dict()
    clip = load_clip(ckpt_paths, embedding_directory, clip_type, model_options=model_options, textmodel_json_config=textmodel_json_config, disable_dynamic=disable_dynamic)
    return clip.patcher


def load_clip(ckpt_paths, embedding_directory=None, clip_type=CLIPType.STABLE_DIFFUSION, textmodel_json_config: str | dict | None = None, model_options=None, disable_dynamic=False):
    if model_options is None:
        model_options = dict()
    clip_data = []
    for p in ckpt_paths:
...
def detect_te_model(sd):
    ...
    if 'model.layers.0.post_feedforward_layernorm.weight' in sd:
        ...
        # Gemma4 12B Unified: 48 layers, layer scalars, and global layers that
        # share K/V instead of storing a separate v_proj.
        if ('model.layers.47.self_attn.q_norm.weight' in sd
                and 'model.layers.0.layer_scalar' in sd
                and 'model.layers.5.self_attn.v_proj.weight' not in sd):
...
def load_text_encoder_state_dicts(state_dicts=[], embedding_directory=None, clip_type=CLIPType.STABLE_DIFFUSION, model_options={}, textmodel_json_config=None,disable_dynamic=False):
    ...
    if len(clip_data) == 1:
        ...
        if te_model == TEModel.CLIP_G:
            ...
            clip_target.clip = sa3.SAT5GemmaModel
            ...
            if clip_type == CLIPType.PIXELDIT:
                clip_target.clip = pixeldit.pixeldit_te(**llama_detect(clip_data))
                clip_target.tokenizer = pixeldit.PixelDiTGemma2Tokenizer
            else:
            ...
            clip_target.clip = lumina2.te(**llama_detect(clip_data), model_type="gemma3_4b_vision")
            clip_target.tokenizer = lumina2.NTokenizer
            tokenizer_data["spiece_model"] = clip_data[0].get("spiece_model", None)
        elif te_model == TEModel.GEMMA_3_12B:
            clip_target.clip = lt.gemma3_te(**llama_detect(clip_data))
            clip_target.tokenizer = lt.Gemma3_12BTokenizer
            tokenizer_data["spiece_model"] = clip_data[0].get("spiece_model", None)
            ...
            if clip_type == CLIPType.HUNYUAN_IMAGE:
                ...
                clip_target.clip = longcat_image.te(**llama_detect(clip_data))
                clip_target.tokenizer = longcat_image.LongCatImageTokenizer
            else:
            ...
            clip_target.clip = gpt_oss.lens_te(**llama_detect(clip_data))
            clip_target.tokenizer = gpt_oss.LensTokenizer
            tokenizer_data["tokenizer_json"] = clip_data[0].get("tokenizer_json", None)
            ...
            if clip_type == CLIPType.IDEOGRAM4 and te_model == TEModel.QWEN3VL_8B:  # Ideogram4 reuses the full Qwen3-VL-8B (13-layer tap for conditioning + multimodal generate).
                clip_data[0] = utils.state_dict_prefix_replace(clip_data[0], {"model.language_model.": "model.", "model.visual.": "visual.", "lm_head.": "model.lm_head."})
                clip_target.clip = ideogram4.te_qwen3vl(**llama_detect(clip_data))
                clip_target.tokenizer = ideogram4.Ideogram4Qwen3VLTokenizer
                ...
                clip_data[0] = utils.state_dict_prefix_replace(clip_data[0], {"model.language_model.": "model.", "model.visual.": "visual.", "lm_head.": "model.lm_head."})
                clip_target.clip = boogu.te(**llama_detect(clip_data))
                clip_target.tokenizer = boogu.BooguTokenizer
            ...
            elif clip_type == CLIPType.MAGE and te_model == TEModel.QWEN3VL_4B:
                clip_data[0] = utils.state_dict_prefix_replace(clip_data[0], {"model.language_model.": "model.", "model.visual.": "visual.", "lm_head.": "model.lm_head."})
                clip_target.clip = mage_flow.te(**llama_detect(clip_data))
                clip_target.tokenizer = mage_flow.MageFlowTokenizer
                ...
                clip_target.tokenizer = flux.KleinTokenizer8B if te_model == TEModel.QWEN3VL_8B else flux.KleinTokenizer
            else:
                clip_data[0] = utils.state_dict_prefix_replace(clip_data[0], {"model.language_model.": "model.", "model.visual.": "visual.", "lm_head.": "model.lm_head."})
                qwen3vl_type = {TEModel.QWEN3VL_4B: "qwen3vl_4b", TEModel.QWEN3VL_8B: "qwen3vl_8b"}[te_model]
                clip_target.clip = qwen3vl.te(**llama_detect(clip_data), model_type=qwen3vl_type)
                clip_target.tokenizer = qwen3vl.tokenizer(model_type=qwen3vl_type)
        elif te_model == TEModel.QWEN3VL_32B:
            clip_target.clip = minimax.te(**llama_detect(clip_data))
            clip_target.tokenizer = minimax.MiniMaxH3Tokenizer
        ...
        if clip_type == CLIPType.SD3:
            ...
            if TEModel.QWEN3_4B in te_models:
                ...
                model_type = "qwen3_2b"
            clip_target.clip = ace15.te(lm_model=model_type, **llama_detect(clip_data))
    ...
    clip = CLIP(clip_target, textmodel_json_config=textmodel_json_config, embedding_directory=embedding_directory, parameters=parameters, tokenizer_data=tokenizer_data, state_dict=clip_data, model_options=model_options, disable_dynamic=disable_dynamic)
    return clip
...
# todo: fix the issues with introducing disable_dynamic to the merge
def load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True, output_clipvision=False, embedding_directory=None, output_model=True, model_options=None, te_model_options=None, disable_dynamic=False):
    ...
    out = load_state_dict_guess_config(sd, output_vae, output_clip, output_clipvision, embedding_directory, output_model, model_options, te_model_options=te_model_options, metadata=metadata, ckpt_path=ckpt_path, disable_dynamic=disable_dynamic)
    if out is None:
    ...
    if out[1] is not None and getattr(out[1], "patcher", None) is not None:
        out[1].patcher.cached_patcher_init = (load_checkpoint_clip_patcher, (ckpt_path, embedding_directory, model_options, te_model_options, disable_dynamic))
...
def load_diffusion_model_state_dict(sd, model_options: dict = None, ckpt_path: Optional[str] = "", metadata: Optional[FileMetadata] = None, disable_dynamic=False):  # load unet in diffusers or regular format
    """
...
def load_vae_patcher(vae_path, metadata=None, device=None, disable_dynamic=False):
    ...
    """
    if metadata is None:
        sd, metadata = utils.load_torch_file(vae_path, return_metadata=True)
    else:
