...
def test_video_preview_compiles_decode(monkeypatch):
    ...
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_enabled", Mock(return_value=True))
    calls = []
    ...
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_begin", begin)
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_end", end)
...
def test_video_preview_leaves_failed_compiler_scope_for_execution_cleanup(monkeypatch):
    ...
    monkeypatch.setattr(latent_preview, "preview_to_image", Mock())
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_enabled", Mock(return_value=True))
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_begin", Mock())
    end = Mock()
    monkeypatch.setattr(latent_preview.model_prefetch, "malloc_graph_end", end)
