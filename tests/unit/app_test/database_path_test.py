...
from comfy.app.database import db
from comfy.cli_args import database_default_path
from comfy.cli_args_types import Configuration
from comfy.execution_context import context_configuration


def test_default_database_url_uses_effective_user_directory(tmp_path):
    user_dir = tmp_path / "custom_user"
    user_dir.mkdir()
    with context_configuration(Configuration(user_directory=str(user_dir), database_url=None)):
        assert db.get_database_url() == f"sqlite:///{user_dir / 'comfyui.db'}"


def test_default_db_path_matches_legacy_default_without_custom_user_directory():
    user_dir = os.path.dirname(database_default_path)
    with context_configuration(Configuration(user_directory=user_dir, database_url=None)):
        assert os.path.abspath(db.get_db_path()) == database_default_path


def test_explicit_database_url_is_preserved():
    database_url = "sqlite:///:memory:"
    with context_configuration(Configuration(database_url=database_url)):
        assert db.get_database_url() == database_url


def test_explicit_url_at_legacy_default_location_is_honoured():
    url = f"sqlite:///{database_default_path}"
    with context_configuration(Configuration(database_url=url)):
...
def test_explicit_database_url_skips_legacy_copy(monkeypatch, tmp_path):
    ...
    with context_configuration(Configuration(database_url=f"sqlite:///{target_db}")):
...
def test_legacy_copy_skipped_when_backup_already_exists(monkeypatch, tmp_path):
    ...
    with context_configuration(Configuration(user_directory=str(user_db.parent), database_url=None)):
...
def test_prepare_file_database_creates_parent_directory(monkeypatch, tmp_path):
    ...
    with context_configuration(Configuration(user_directory=str(db_path.parent), database_url=None)):
...
def test_prepare_file_database_accepts_relative_database_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(db, "copy_legacy_default_db", lambda path: None)
    with context_configuration(Configuration(database_url="sqlite:///relative.db")):
